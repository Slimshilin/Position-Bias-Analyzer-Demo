...
def preprocess(data_file, benchmark, col_name, failure_cnt, fout):
    '''
    Purpose:
    ...
    stats = length_statistics_inferin(data, benchmark)
    ...
    data['extracted'] = [match_answer(ans, benchmark) for ans in data[col_name]]
    ...
    ### Example ###
    # DevBench (single round QA): data.columns now should becomes ['cmp_index', 'question', 'answer1', 'answer2', 'A', 'B', 'reference_answer', 'evaluating_guidance', 'task', 'gpt-3.5-turbo-1106', 'extracted']
    # MTBench (2-round QA): data.columns now should become ['cmp_index', 'question_1', 'question_2', 'answer1_1', 'answer1_2', 'answer2_1', 'answer2_2', 'A', 'B', 'reference_answer_1', 'reference_answer_2', 'evaluating_guidance', 'task', 'gpt-3.5-turbo-1106', 'extracted']
...
def analyze(data, refm, langs, tasks, fout, data_root):
    ...
    wr_table = defaultdict(lambda: [-1] * len(models))  # Win rate table.
    wd2_table = defaultdict(lambda: [-1] * len(models))  # Win + Draw/2 rate table.
    wb_table = defaultdict(lambda: [-1] * len(models))  # Win + Both good rate table.
    ...
    for i, m in enumerate(models):
        ...
        for lang in langs:
            ...
            wr_table[lang_name][i] = wr.at[m, refm] if m != refm else None
            wd2_table[lang_name][i] = wr.at[m, refm] + dr.at[m, refm] / 2. if m != refm else None
            wb_table[lang_name][i] = wr.at[m, refm] + wff.at[m, refm] if m != refm else None

        # Does the same calculation for taskbilities.
        for task in tasks:
            wr, wff, lff = calc_win_rate(data, models, task=task)
            dr = wff + lff
            wr_table[task][i] = wr.at[m, refm] if m != refm else None
            wd2_table[task][i] = wr.at[m, refm] + dr.at[m, refm] / 2. if m != refm else None
            wb_table[task][i] = wr.at[m, refm] + wff.at[m, refm] if m != refm else None
...
def analyze_pipe(data_file, benchmark, refm=None, col_name='gpt-4-1106-preview', failure_cnt=5):
    # Opens a log file for writing.
    log_file = osp.join(osp.dirname(data_file), 'log.txt')
    fout = open(log_file, 'w')
    # Preprocesses the data.
    data, _ = preprocess(data_file, benchmark, col_name, failure_cnt, fout)
