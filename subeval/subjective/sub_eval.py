...
def subeval_call():
    ...
    for m in args.model:
        data = generate_inference_input(data_file, benchmark=args.benchmark, model=m, refm=args.refm, mode=args.mode, fill_contents=args.fill_contents, seed=args.seed)
        data_all.append(data)
    data_all = pd.concat(data_all)
    dump(data_all, target_file)
    
    # Do judgement/evaluation (see judge_util.py for more detail)
    infer_result_file = judge_infer(target_file, benchmark=args.benchmark, judge=args.judge, nopt=args.eval_nopt, nproc=args.eval_proc, failure_cnt=10)
    
    # Analyze (see analyze_util.py for more detail)
    analyze_pipe(infer_result_file, benchmark=args.benchmark, refm=args.refm, col_name=args.judge, failure_cnt=10)
