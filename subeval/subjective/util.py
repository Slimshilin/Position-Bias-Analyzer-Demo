...
def length_statistics(data):
    keys = data.keys()
    keys = [k for k in keys if k.startswith('answer_1-') or k.startswith('answer_2-')]
    import tiktoken
...
def length_statistics_inferin(data, benchmark):
    lt = len(data)
    length_map = defaultdict(list)
    import tiktoken
    enc = tiktoken.get_encoding('cl100k_base')
    ...
    if benchmark == "MTBench":
        for i in range(lt):
            item = data.iloc[i]
            length_map[item['A']].append(len(enc.encode(str(item['answer1_1']))))
            length_map[item['A']].append(len(enc.encode(str(item['answer1_2']))))
            length_map[item['B']].append(len(enc.encode(str(item['answer2_1']))))
            length_map[item['B']].append(len(enc.encode(str(item['answer2_2']))))
